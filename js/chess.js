...
class Move {
    ...
    castle(r, f) {
        ...
        // king has moved?
        if (this.chess.getPiece(Chess.rankIdx(r), FILES[f]).piece.hasMoved())
          [q,k] = [false,false];
        
        // king side
        for (let i = f+1; i < WIDTH-1; i++) {
          if (this.chess.getPiece(Chess.rankIdx(r), FILES[i]).piece.isSet())
            k = false;
        }
        // queen side
        for (let i = f-1; i > 0; i--) {
        ...
        let [krook,qrook] = 
          [
            this.chess.getPiece(Chess.rankIdx(r), FILES[WIDTH-1]),
            this.chess.getPiece(Chess.rankIdx(r), FILES[0])
          ];
...
class Chess {
    ...
    getPiece(rank, file) { return this.board[Chess.rankIdx(rank)][Chess.fileIdx(file)]; }
    ...
    populated(s) { return (Chess.inbounds(s.r, s.f) && this.getPiece(Chess.rankIdx(s.r), FILES[s.f]).piece.isSet()); }
...
getAvailableMoves(square) {
    ...
    available = Object.keys(group).reduce((prev, curr) => {
      ...
      for (let m of group[curr].m) {
        ...
        let target = this.getPiece(rank, file);
  ...
  _move(source, target) {
    ...
    if (available.find(a => (((a.rank) === target.rank) && ((a.file) === target.file)))) {
      ...
      if (action === actions.CASTLE) {
        // get new locations
        const kingSquare = this.getPiece(source.rank+modifiers.source.rankIdx, FILES[Chess.fileIdx(source.file)+modifiers.source.fileIdx]);
        const rookSquare = this.getPiece(target.rank+modifiers.target.rankIdx, FILES[Chess.fileIdx(target.file)+modifiers.target.fileIdx]);
        // assign pieces to squares
        kingSquare.piece = this.getPiece(source.rank, source.file).piece;
        rookSquare.piece = this.getPiece(target.rank, target.file).piece;
  ...
  move(from, to) {
    ...
    this._move(this.getPiece(fromRank, fromFile), this.getPiece(toRank, tofile));
  }
}
